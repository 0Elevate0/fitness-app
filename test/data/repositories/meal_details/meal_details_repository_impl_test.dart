...
import 'package:fitness_app/data/repositories/meal_details/meal_details_repository_impl.dart';
import 'package:fitness_app/domain/entities/meal_details/meal_details_entity.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mockito/annotations.dart';
import 'package:mockito/mockito.dart';

import 'meal_details_repository_impl_test.mocks.dart';

@GenerateMocks([MealDetailsRemoteDataSource])
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();
  test(
    'when call getMealDetails it should be called successfully from MealDetailsRemoteDataSource and returns a List of MealCategoryEntity',
    () async {
      // Arrange
      final MockMealDetailsRemoteDataSource mockedMealDetailsRemoteDataSource =
          MockMealDetailsRemoteDataSource();
      final mealDetailsRepositoryImpl = MealDetailsRepositoryImpl(
        mockedMealDetailsRemoteDataSource,
      );
      ...
      when(
        mockedMealDetailsRemoteDataSource.getMealDetails(
          mealId: anyNamed("mealId"),
        ),
      ).thenAnswer((_) async => expectedResult);

      // Act
      final result = await mealDetailsRepositoryImpl.getMealDetails(
        mealId: "1234",
      );
