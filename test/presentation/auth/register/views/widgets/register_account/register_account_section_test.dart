...
import 'register_account_section_test.mocks.dart';
...
void main() {
      ...
      builder: (context, child) {
        return MaterialApp(
          home: BlocProvider<RegisterCubit>.value(
            ...
            child: const Scaffold(body: RegisterAccountSection()),
          ),
        );
      },
    );
  ...
  testWidgets("Verifying RegisterAccountSection Widgets", (tester) async {
    // Act
    ...
    expect(
      find.byWidgetPredicate(
        (widget) =>
            ...
            widget.children.elementAt(1) is RPadding,
      ),
      findsOneWidget,
    );
    ...
    expect(
      find.byWidgetPredicate(
        (widget) => widget is RPadding && widget.child is Column,
      ),
      findsNWidgets(2),
    );
    expect(find.byType(Text), findsAny);
    expect(find.text(AppText.heyThere.tr()), findsOneWidget);
    expect(find.text(AppText.createAnAccount.tr()), findsOneWidget);
    expect(find.text(AppText.register.tr()), findsNWidgets(2));
    ...
    expect(find.byType(HaveAccountRow), findsOneWidget);
  });
