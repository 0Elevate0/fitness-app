...
import 'package:fitness_app/presentation/auth/register/views/widgets/goal/goal_choices.dart';
...
import 'goal_section_test.mocks.dart';
...
void main() {
      ...
      builder: (context, child) {
        return MaterialApp(
          home: BlocProvider<RegisterCubit>.value(
            ...
            child: const Scaffold(body: GoalSection()),
          ),
        );
      },
    );
  ...
  testWidgets("Verifying GoalSection Widgets", (tester) async {
    // Act
    ...
    expect(find.byType(GoalChoices), findsOneWidget);
    expect(find.byType(GoalNextButton), findsOneWidget);
    expect(find.text(AppText.goalTitle.tr()), findsOneWidget);
