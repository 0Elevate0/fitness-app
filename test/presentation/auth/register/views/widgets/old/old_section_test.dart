...
import 'old_section_test.mocks.dart';
...
void main() {
      ...
      builder: (context, child) {
        return MaterialApp(
          home: BlocProvider<RegisterCubit>.value(
            ...
            child: const Scaffold(body: OldSection()),
          ),
        );
      },
    );
  ...
  testWidgets("Verifying OldSection Widgets", (tester) async {
    // Act
    ...
    expect(find.byType(YearsOldChoice), findsOneWidget);
    expect(find.byType(NextButton), findsOneWidget);
    expect(find.text(AppText.oldTitle.tr()), findsOneWidget);
