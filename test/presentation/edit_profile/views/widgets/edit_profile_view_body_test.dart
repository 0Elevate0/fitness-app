...
import 'edit_profile_view_body_test.mocks.dart';
...
void main() {
  ...
  testWidgets("Verifying EditProfileViewBody Widgets at Initial State", (
    tester,
  ) async {
    // Act
    ...
    expect(find.byType(EditProfileDetailsSection), findsOneWidget);
  });

  testWidgets(
    "Verifying EditProfileViewBody Widgets On currentSection == editWeight",
    (tester) async {
      // Arrange
      when(mockEditProfileCubit.state).thenReturn(
        const EditProfileState(
          currentSection: EditProfileSection.editWeight,
          selectedWeight: 60,
        ),
      );
      ...
      expect(find.byType(EditWeightSection), findsOneWidget);
    },
  );

  testWidgets(
    "Verifying EditProfileViewBody Widgets On currentSection == editGoal",
    (tester) async {
      // Arrange
      when(mockEditProfileCubit.state).thenReturn(
        EditProfileState(
          currentSection: EditProfileSection.editGoal,
          selectedGoal: FitnessMethodHelper.goals[0],
        ),
      );
      when(mockEditProfileCubit.stream).thenAnswer(
        (_) => Stream.fromIterable([
      ...
      expect(find.byType(EditGoalSection), findsOneWidget);
    },
  );

  testWidgets(
    "Verifying EditProfileViewBody Widgets On currentSection == editActivity",
    (tester) async {
      // Arrange
      when(mockEditProfileCubit.state).thenReturn(
        const EditProfileState(currentSection: EditProfileSection.editActivity),
      );
      when(mockEditProfileCubit.stream).thenAnswer(
        (_) => Stream.fromIterable([
          const EditProfileState(
            currentSection: EditProfileSection.editActivity,
          ),
        ]),
      );
      // Act
      ...
      expect(find.byType(EditActivitySection), findsOneWidget);
    },
  );
