...
import 'recommendation_list_test.mocks.dart';
...
void main() {
  ...
  testWidgets("Verifying RecommendationList Widgets on Initial state", (
    tester,
  ) async {
    // Act
  ...
  testWidgets(
    "Verifying RecommendationList Widgets on Success state With data",
    (tester) async {
      // Arrange
      when(mockHomeCubit.state).thenReturn(
        const HomeState(
          recommendationStatus: StateStatus.success([
      ...
      expect(find.byType(RecommendationItem), findsWidgets);
      expect(
        find.byWidgetPredicate(
  ...
  testWidgets(
    "Verifying RecommendationList Widgets on Success state Without data",
    (tester) async {
      // Arrange
      when(mockHomeCubit.state).thenReturn(
        const HomeState(recommendationStatus: StateStatus.success([])),
      );
      when(mockHomeCubit.stream).thenAnswer(
        (_) => Stream.fromIterable([
          const HomeState(recommendationStatus: StateStatus.success([])),
      ...
      expect(
        find.text(AppText.emptyExercisesRecommendationMessage.tr()),
        findsOneWidget,
      );
    },
  );
